...
class C45:
    ...
    def info(self, res_col):
        ...
        for v in self.uniques(res_col):
            p = self.freq(res_col, v) / self.flen(res_col)
            s += p * math.log(p, 2)
        return -s
    ...
    def infox(self, col, res_col):
        ...
        """
        s = 0 # sum
        for subt in self.partitionOnColumn(col):
    ...
    def partitionOnColumn(self, col):
        """ Returns subtables divided by values of the column _col_.
        """
        return [self.partition(col, v) for v in self.uniques(col)]

    #--------------------------------------------------------------------------

    def divide(self, res_col):
        tree = Node(self)
        ...
        for subt in self.partitionOnColumn(col):
            if subt.isHomogeneous(res_col):
                ...
                tree.add(subt.divide(res_col))
        return tree

    @classmethod
    def makeDecisionTree(cls, arrayOfDictionaries, res_col):
        ...
        """
        return C45.fromTable(arrayOfDictionaries).divide(res_col)
